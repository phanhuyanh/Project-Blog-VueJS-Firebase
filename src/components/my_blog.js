...
export default {
  ...
  methods: {
    searchBlog(data) {
      ...
      this.tempListBlog = this.listBlog.filter(
        e => e.title.includes(data) || e.description.includes(data)
      );
    },
    nextPage(idx) {
      this.pageCur = idx;
  ...
  computed: {
    ...
    createPag() {
      ...
      if (this.page === 1) {
        ...
        else if (this.page + 2 < num_page)
          this.listNav.push(this.page + 1, "...", num_page);
      }
